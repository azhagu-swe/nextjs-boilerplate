...
export function MainLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh', flexDirection: 'column' }}> {/* Ensure column flex and min height */}
    <CssBaseline />
    <TopAppBar open={open} handleDrawerOpen={handleDrawerOpen} title="Tutorial App" />
    <Box sx={{ display: 'flex', flexGrow: 1 }}> {/* Inner flex row for drawer/main */}
